...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    bytes_transferred = 2048 * 4; 
    time_spent = (float)(stop_time - start_time) / 125e6; 
    printf("You transferred %d bytes of data in %f seconds\n",bytes_transferred,time_spent);
    throughput = (bytes_transferred / (1024.0 * 1024.0)) / time_spent;
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Jedidiah Agbenu - Custom Peripheral Demonstration\n\r");
